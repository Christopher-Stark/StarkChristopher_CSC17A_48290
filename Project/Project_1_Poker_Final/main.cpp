 ...
 * Purpose: Project 1 Assignment, Video Poker Jacks or Better!
 */
...
void binGetRecords(string fileName, Player *a);
void recordUpdate(string fileName, Player *a);
void binRecordUpdate(string fileName, Player *a);
int byteNum(int index);
void randomAccessStats(string fileName, const Player *a);
void showRandomAccessStats(const Player *a);
...
int main(int argc, char** argv)
{
    ...
    binRecordUpdate(binFile, player);
    //binGetRecords(binFile, player); **NOT WORKING PROPERLY**
    ...
    do
    {
        ...
        switch (menu)//Switch for menu options
        {
            case 1:
                ...
                binRecordUpdate(binFile, player);//Updates bin file with records
                menuReturn(); //a pseudo pause function for clearing screen when hand is over
                break;
            case 2:
                //**ONLY USED AS VERIFICATION FOR BINARY FILE COMPARISON**
                //stats(player, name); Used Stats file to get information 
                binRecordUpdate(binFile, player);
                //randomAccessStats(binFile, player); **NOT WORKING**
                menuReturn();
                break;
            case 3:
...
void displayMenu(const Player *a, int &menu)
{
    ...
    while ((input > 4 || input < 1) && a->credits != 0);//Error checking for credits & menu
    if (a->credits == 0)//If user runs out of credits
    {
        cout << "You have ran out of credits, better luck next time!" << endl;
    }
}
/**
 * Gets user name adds to record array, creates dealt hand with hand/suit array
 * @param a
 * @param index
 * @param name
 * @return struct object with dealt hand, user name and index location
 */
Player *hand(Player *a, int &index, string &name)
{
...
{
    int index = -1;
    while (!name.size() || name.size() != 4)
    {
        if (a->record[9].name != "Name")
        {
            cout << "Records are full, please choose a name from the list!" << endl;
            for (int i = 0; i < a->crdData.rcdSize; i++)
            {
                cout << a->record[i].name << endl;
            }
        }
        cout << "\nPlease enter your user name that is 4 chars long: ";
        cin >> name;
    }
    for (int i = 0; i < a->crdData.rcdSize; i++)
    {
    ...
    if (index >= 0)//Displays records for player of that index
    {
...
/**
 * Determines cursor location for random access file
 * @param index
 * @return 
 */
int byteNum(int index)
...
/**
 * Sorts player hand for easier win condition determination
 * @param card
 * @param b
 * @param suit
 */
...
/**
 * Gets records from non binary file and updates struct records array with info
 * @param fileName
 * @param a
 */
void getRecords(string fileName, Player *a)
...
/**
 * **DOES NOT WORK** Gets records from binary file and updates struct records array 
 * with info
 * @param fileName
 * @param a
 */
void binGetRecords(string fileName, Player *a)
{
    ...
    {
        fileIn.read(reinterpret_cast<char*> (a->record[i]), sizeof (a->record[i]));
        i++;
    }*/
...
/**
 * Writes struct record array to binary file
 * @param fileName
 * @param a
 */
void binRecordUpdate(string fileName, Player *a)
{
    ...
    for(int i = 0; i < a->crdData.rcdSize; i++)
    {
    fileOut.write(reinterpret_cast<char*> (&a->record[i]), sizeof (a->record[i]));
    }

    fileOut.close();
}
/**
 * **NOT WORKING** Tried to get this working and failed, is suppose to grab struct
 * records written to binary file and determine cursor location off user name index
 * @param fileName
 * @param a
 */
void randomAccessStats(string fileName, const Player *a)
{
    ...
    int size = 24, spot, num;//Size of all array information, cursor pos and index
    //Was using these variables to test what was being grabbed from the file
    //It never grabbed what i wanted it to so i left them here to show I was working on it
    //If it did grab what I wanted then I would have displayed the records to the user
    //Via switch menu 2..... I commented everything out that wasnt working in main
    //and commented
    char name, crdWon, crdLst, hndWon, hndLst;
    //cout << "What user name stats would you like to see 1-10: ";
    //cin >> num;
    //while (num < 1 || num > 10)
    //{
    // cout << "What user name stats would you like to see 1-10: ";
    ...
    //num -= 1;
    //spot = size*num;
    file.seekg(20L, ios::beg);//Hard coded cursor location to test info grabbed
    file.read(reinterpret_cast<char *> (&name), sizeof (int));
    cout << "Binary Name: " << name << endl;
    //showRandomAccessStats(test); **NOT WORKING**
    file.close();
}
/**
 * **NOT WORKING** was going to supplement random access function with display
...
{
    cout << fixed << setw(10);
    cout << "Name: " << a->record->name << endl;
    cout << "Credits won: " << a->record->crdWon << endl;
    cout << "Credits lost: " << a->record->crdLost << endl;
    cout << "Hands won: " << a->record->hndWon << endl;
    cout << "Hands lost: " << a->record->hndLost << endl;
