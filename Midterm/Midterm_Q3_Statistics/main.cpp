/* 
 * File:   main.cpp
 * Author: Christopher Stark / Dr. Mark E. Lehr
 * Created on October 19, 2022, 8:09 AM
 * Purpose: Takes Dr.Lehrs StatStructureStub code and tests modified Stats *stat n
...
Stats *stat(const Array *array)
{
    ...
    for (int i = 0; i < array->size; i++)//Calculates average
    {
        stats->avg += array->data[i];
    }
    stats->avg /= array->size;

    if (array->size % 2 == 0)//Calculates median
    {
    ...
    int freq = 1;//Base variables for keeping track of Max Freq and # modes
    int finFreq = 1;
    int mode = 0;
    for (int i = 0; i < array->size - 1; i++)//Determines max Freq. and number of modes
    {
        if (array->data[i] == array->data[i + 1])
        {
            freq++;
            if (i == (array->size - 2) && freq == finFreq)
            {
                mode++;
            }
        }
    ...
    int num;//Keeps track of current sorted data array index number
    int count = 0;//Keeps track of mode data array index
    stats->modFreq = finFreq;//Sets max freq
    stats->mode->size = mode;//Sets number of modes
    if (stats->mode->size != 0)//Creates array if mode set is needed
    {
        stats->mode->data = new int[stats->mode->size]; //Mode set array

        for (int i = 0; i < array->size - 1; i++)//Fills mode set
        {
            if (array->data[i] == array->data[i + 1])
            {
                num = array->data[i];
                if (i == (array->size - 2) && num < mode)
                {
                    stats->mode->data[count] = num;
                }
            }
            else
            {
